...
C#######################################################################
      ...
      use treecode, only: schg,x,y,z,numpars
      IMPLICIT NONE
C
...
C local variables
      ...
      IF (p%exist_ms .EQ. 0 .and. ifirst==0) THEN
         ...
         IF (err .NE. 0) THEN
            ...
            STOP
         END IF
         CALL COMP_MS(p,x,y,z,schg(:,:,1),numpars)
         p%exist_ms=1
...
C box use the expansion for the approximation.
	...
	!pause
     ...
     &    (p%numpar .GT. 40)) THEN
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@         
C        IF (p%exist_ms .EQ. 0) THEN
C		ALLOCATE(p%ms(16,0:torder,0:torder,0:torder),STAT=err)
C		ALLOCATE(p%ms(0:torder,0:torder,0:torder,16),STAT=err)
C		IF (err .NE. 0) THEN
C			WRITE(6,*) 'Error allocating node moments! '
C			STOP
C		END IF
C#####################################################################
C Generate the moments if not allocated yet 
C		CALL COMP_MS(p,x,y,z,q(:,:,1),numpars)
C#####################################################################			
C		p%exist_ms=1
C	END IF   
...
C MPI variables
      ...
      stime = MPI_Wtime();
      DO i=1,(ie+1)
            id = (i-1)*numprocs+myid+1;
            if(id < (numpars+1)) then
		      tarpos(1)=x(id)
		      tarpos(2)=y(id)
		      tarpos(3)=z(id)
		      tarq=tr_q(:,id)
		      tempq=tchg(id,:,:)
		      peng_old(1)=tpoten_old(id)
		      peng_old(2)=tpoten_old(id+numpars)
		      peng=0.d0
C Remove the singularities 
                  tempx=x(id)
                  temp_area=tr_area(id)
                  x(id)=x(id)+100.123456789d0
                  tr_area(id)=0.d0	
		      CALL COMPP_TREE(p,peng,x,y,z,schg,tpoten_old,
     &		kappa,theta,numpars,kk,eps,tempq,der_cof)
		      mytpoten(id)=pre1*peng_old(1)-peng(1)
		      mytpoten(numpars+id)=pre2*peng_old(2)-peng(2)
                  x(id)=tempx
