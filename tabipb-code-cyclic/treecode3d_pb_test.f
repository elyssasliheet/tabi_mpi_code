...
C###################################################
      ...
      !print*, "LOCAL BEG", local_beg
      !print*, "LOCAL END", local_end
C      call MPI_COMM_RANK(MPI_COMM_WORLD, my_rank, ierr)
C      call MPI_COMM_SIZE(MPI_COMM_WORLD, num_process, ierr)
C      print *,'my_rank=',my_rank,'num_process= ', num_process

C      local_numpars=numpars/num_process+1
C      local_beg=1+my_rank*local_numpars
C      local_end=local_beg+local_numpars-1
C      if (local_end > numpars) local_end=numpars
C      print *,my_rank,local_beg,local_end,local_end-local_beg+1
C####################################################
...
C		print *,'cpu time for one p-c intercation: ',i,real(time2-time1)
	ENDDO
      ...
      call MPI_Reduce(mytpoten, final_tpoten, 2*numpars, MPI_REAL8, 
     & MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      ...
      if (myid==0) then
            do isrc=1,numprocs-1
               call MPI_SEND(tpoten,2*numpars,MPI_REAL8,isrc,
     &              tag,MPI_COMM_WORLD,ierr)
            enddo
      else
            call MPI_RECV(tpoten,2*numpars,MPI_REAL8,0,tag,
